        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="card-title d-inline-block">{{ $keywords['Offline_Gateways'] ?? __('Offline Gateways') }}</div>
                        </div>
                        <div class="col-lg-6 mt-2 mt-lg-0">
                            <a href="#" class="btn btn-primary float-lg-right float-left btn-sm" data-toggle="modal"
                                data-target="#createModal"><i class="fas fa-plus"></i> {{ $keywords['Add_Gateway'] ?? __('Add Gateway') }}</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (count($ogateways) == 0)
                                <h3 class="text-center">{{ $keywords['NO_OFFLINE_PAYMENT_GATEWAY_FOUND'] ?? __('NO OFFLINE PAYMENT GATEWAY FOUND') }}</h3>
                            @else
                                    ...
                                    <table class="table table-striped mt-3" id="basic-datatables">
                                        <thead>
                                            <tr>
                                                ...
                                                @if ($shopsettings->is_shop == 1)
                                                    <th scope="col">{{ $keywords['Status'] ?? __('Status') }}</th>
                                                @endif
                                                <th scope="col">{{ $keywords['Serial Number'] ?? __('Serial Number') }}</th>
                                                    ...
                                                    @if ($shopsettings->is_shop == 1)
                                                        <td>
                                                            <form id="ItemForm{{ $ogateway->id }}" class="d-inline-block"
                                                                action="{{ route('user.offline.status') }}"
                                                                method="post">
                                                                @csrf
                                                                ...
                                                                <select
                                                                    ...
                                                                    <option value="1"
                                                                        {{ $ogateway->item_checkout_status == 1 ? 'selected' : '' }}>
                                                                        {{ $keywords['Active'] ?? __('Active') }}</option>
                                                                    <option value="0"
                                                                        {{ $ogateway->item_checkout_status == 0 ? 'selected' : '' }}>

...
@section('meta-description', $preview == null ? $details->meta_description : $preview->meta_description)
@section('meta-keywords', $preview == null ? $details->meta_keywords : $preview->meta_keywords)

@section('styles')
<link rel="stylesheet" href="{{ asset('assets/front/css/summernote-content.css') }}">
<style>
  .header_v1 .top_header, .brand_logo, .nav-menu, .nav-push-item, .navbar-toggler{
  ...
  .header_navigation .nav-container::after{
    position: absolute;
    content: "Preview";
    left: 50%;
    top: 50%;
...
@section('content')
  ...
  <section class="olima_breadcrumb bg_image lazy" @if (!empty($bgImg)) data-bg="{{ $bgImg->breadcrumb != null ? Storage::url($bgImg->breadcrumb) : asset('assets/admin/img/noimage.jpg') }}" @endif>
      ...
      <div class="row align-items-center">
        <div class="col-lg-7">
          <div class="breadcrumb-title"> 
            <h1>{{ $preview == null ? $details->title : $preview->title}}</h1>
          </div>
        </div>
        ...
        <div class="col-lg-8">
          <div class="blog_details_wrapper">
            <div class="post_content pb-70">
              @php
                  $sldImgs = !empty($details->post->slider_images) ? json_decode($details->post->slider_images, true) : [];
              @endphp

              <div class="img-wrapper">
                  <img style="width:100%; height:auto" 
                      src="{{ !empty($sldImgs) && isset($sldImgs[0]) ? Storage::url($sldImgs[0]) : asset('assets/admin/img/noimage.jpg') }}" 
                      class="lazy" 
                      alt="slider image">
              </div>

              <h3>{{ $preview == null ? $details->title : $preview->title }}</h3>

              <div class="post_meta">
                <span class="calender">{{ date_format($details->created_at, 'F d, Y') }}</span>
                <span class="writer">{{ $preview == null ? $details->author : $preview->author }}</span>
              ...
              <div class="summernote-content">{!! !empty($preview == null ? $details->content : $preview->content) ? replaceBaseUrl($preview == null ? $details->content : $preview->content, 'summernote') : null !!}</div>
            </div>
                ...
                <div class="col-lg-12">
                  <div class="social_box float-left">
                    <ul>
                      ...
                      <li><a href="//www.linkedin.com/shareArticle?mini=true&amp;url={{urlencode(url()->current()) }}&amp;title={{ $preview == null ? $details->title : $preview->title}}"><i class="fab fa-linkedin-in"></i></a></li>
                    </ul>
                  </div>
                </div>
              </div>
                  ...
                  @foreach ($relatedPosts as $post)
                    <div class="grid_item">
                      ...
                      <div class="post_content">
                        ...
                        <h3>
                          <a href="">{{ strlen($post->title) > 30 ? mb_substr($post->title, 0, 30, 'UTF-8') . '...' : $post->title }}</a>
                        </h3>
                      </div>
                    </div>
                  @endforeach
